...
void Robot::Start()
{
  ...
  //************************** HEAD CODE ************************
  head.Emotion_Setup(0x70);
  head.Ultrasonic_Setup();
  head.servo_setup(3);
  head.Emotion_SetMode(1);
  head.Emotion_Show();
  head.HexaHead::Buzzer_Setup(2);
  head.sing(S_connection);
...
//************************** HEAD CODE ************************
void RobotAction::Expression(int expId)
{
  Serial.print("EXP: ");
  Serial.println(expId);
    robot.head.Emotion_SetMode(expId);
}

void RobotAction::ObjAvoid(bool state)
{
   objectAvoidance = state;
   if (state) {
      //reset sensor
      robot.head.dist_fwd = 0;
      robot.head.Buzzer_Beep();
      robot.head.Wake();
   }
   else  
   {
