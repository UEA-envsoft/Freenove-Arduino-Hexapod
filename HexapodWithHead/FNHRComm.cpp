...
int objAvoidLeftCount = 0; 
int objAvoidRightCount = 0; 
...
void Communication::UpdateESP8266()
{
  ...
  if (ReceiveDataESP8266(&esp8266ClientID, esp8266InData, &esp8266InDataCounter))  // Received order.
  {
    ...
    {
      //if it is a partial order
      Serial.print("partial order ");
      Serial.println(sizeof(esp8266InData));
      if (sizeof(esp8266InData) > 2 && esp8266InData[1] == Orders::requestChangeIO && esp8266InData[2] == 7) robotAction.ObjAvoid(false);      
    }
  }
  //************************** HEAD CODE ************************
  else if (blockedOrder == 0 && robotAction.objectAvoidance)
  {
    Serial.print("dist LMR ");
    Serial.print(robotAction.robot.head.dist_left);
    Serial.print("  ");
    Serial.print(robotAction.robot.head.dist_fwd);
    Serial.print("  ");
    Serial.println(robotAction.robot.head.dist_right);



    if (robotAction.robot.head.dist_fwd == 0) 
    {
      //waiting for data
      //robotAction.robot.head.sing(S_confused);
      return;
    }
    //turn off avoidance if sensor blocked
    if (robotAction.robot.head.dist_fwd == -1) 
    {
      robotAction.robot.head.sing(S_buttonPushed);
      robotAction.ObjAvoid(false);
      //reset sensor
      robotAction.robot.head.dist_fwd = 0;
      return;
    }
  
    //do obj avoid   
    if (objAvoidLeftCount > 0)
    {
     // robotAction.robot.head.sing(S_happy_short);
      objAvoidLeftCount++ ;
      Serial.print("turn left ");      
      Serial.println(objAvoidLeftCount);     
      if (objAvoidLeftCount > 8) objAvoidLeftCount = 0; 
      byte avData[] = {128,88,129,1,129};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }
    else if (objAvoidRightCount > 0)
    {
      //robotAction.robot.head.sing(S_happy_short);
      objAvoidRightCount++ ;
      Serial.print("turn right ");      
      Serial.println(objAvoidRightCount);     
      if (objAvoidRightCount > 8) objAvoidRightCount = 0; 
      byte avData[] = {128,90,129,1,129};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }
    //if within 10cm back up
    else if (robotAction.robot.head.dist_fwd < 10)
    {      
      Serial.println("back up");
      byte avData[] = {128,82,129,1,129};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }    
    //if we are within 16 cm turn on the spot
    else if (robotAction.robot.head.dist_fwd < 20)
    {
      //
      if (robotAction.robot.head.dist_left > robotAction.robot.head.dist_right)
      {
        Serial.println("turn left");      
        robotAction.robot.head.sing(S_mode1);
        objAvoidLeftCount++;
        byte avData[] = {128,88,129,1,129};
        OrderSource os = OrderSource::FromNone;
        Communication::HandleOrder(avData, os);
      ...
      else
      {
        ...
        robotAction.robot.head.sing(S_mode2);
        objAvoidRightCount++;
        byte avData[] = {128,90,129,1,129};
        OrderSource os = OrderSource::FromNone;
        Communication::HandleOrder(avData, os);
    ...
    //dont get too close to a wall
    else if (robotAction.robot.head.dist_right < 14)
    {
      Serial.println("head to the left"); 
      byte avData[] = {128,84,129,1,129};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }
    else if (robotAction.robot.head.dist_left < 14)
    {
      Serial.println("head to the right"); 
      byte avData[] = {128,86,129,1,129};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }
    else  //straight ahead
    {
      Serial.println("FWD");
      //robotAction.Expression(1);
      byte avData[] = {128,80,129,129,0};
      OrderSource os = OrderSource::FromNone;
      Communication::HandleOrder(avData, os);
    }
  }
...
//    Serial.println(inData[4]);
  ...
  //************************** HEAD CODE ************************
    //pins are being used so do not want to simply change state
    //digitalWrite(pins[inData[2]], inData[3]);

   // const int pins[8] = { 20,21,A0,A1,15,14,2,3 };
   // 20 is happy expression
   // 21 is angry expression
   // 15 is supr expression
   // 2 is sad exp
   // 14 is obj avoid on
   // 3 is obj avoid off
   if (inData[2] == 0) robotAction.Expression(1);
   if (inData[2] == 1) robotAction.Expression(2);
   if (inData[2] == 4) robotAction.Expression(3);
   if (inData[2] == 5) robotAction.ObjAvoid(true);
   if (inData[2] == 6) robotAction.Expression(4);
   if (inData[2] == 7) robotAction.ObjAvoid(false);
